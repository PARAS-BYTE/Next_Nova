  ...
  const handleAiGrade = async (submission: Submission) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `/api/assignments/${id}/ai-grade/${submission._id}`,
        {},
        {
          withCredentials: true,
          headers: token ? { Authorization: `Bearer ${token}` } : {},
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8" style={{ background: palette.bg }}>
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
        <Button variant="ghost" size="sm" className="w-full sm:w-auto" style={{ color: palette.text }} onClick={() => router.push('/admin/assessments')}>
